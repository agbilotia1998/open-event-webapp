...
function emailSend(toEmail, url, appName) {
 ...
 if(url){
    ...
    downloadLinkExpiryDate.setTime(currDate.getTime() + 7200000); // corresponds to current date + 2 hours
    uploadFailureMesg = '. There was some problem with the cloud upload of the event folder. Please download the zip immediately. It will be purged soon from the generator server';
}
else {
...
const emailContent = 'Hi ! <br>' +
    ...
    'You can download a zip of your website from  ' + ' here'.link(downloadUrl) + '. The download link for zip expires on ' + downloadLinkExpiryDate + uploadFailureMesg +
    '<br><br><br>' +
